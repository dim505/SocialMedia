...
";
        ...
        public IActionResult GetPosts(string TypeOfPost, string ViewUserProfile)
        {
            ...
            using (IDbConnection db = new SqlConnection(ConnStr))
            {
                ...
                {
                    SQL = "select * from ( select Auth0IDAuthor, a.PostGuid, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, DateCreated, PostContent, count (Auth0IDWhoLiked) as PostLikeCount, DidUserLikePost, DisableSharing, DisableComments, FileUrl, FileType, TempTagList from ( select Auth0IDAuthor, Post.PostGuid, DateCreated, PostContent, Auth0IDWhoLiked, case when Auth0IDAuthor = Auth0IDWhoLiked then 'yes' else null end as DidUserLikePost, case when post.DisableSharing <> 'True' then 'False' end as DisableSharing, case when post.DisableComments <> 'True' then 'False' end as DisableComments from SM_Posts Post left join ( select * from SM_LikeCommentTable ) LikedComm on Post.Auth0IDAuthor = LikedComm.Auth0IDWhoLiked and Post.PostGuid = LikedComm.PostGuid ) a inner join SM_Account_Info AcctInfo on Auth0IDAuthor = AcctInfo.Auth0ID LEFT join SM_Follow_Following_Table on Auth0IDAuthor = SM_Follow_Following_Table.FollowerAuth0ID left join SM_Posts_File_Urls ON a.PostGuid = SM_Posts_File_Urls.PostGuid where Auth0IDAuthor = @Auth0IDAuthor group by Auth0IDAuthor, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, a.PostGuid, DateCreated, PostContent, DidUserLikePost, DisableSharing, DisableComments, FileUrl, FileType, TempTagList union select Auth0IDAuthor, a.PostGuid, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, DateCreated, PostContent, count (Auth0IDWhoLiked) as PostLikeCount, DidUserLikePost, DisableSharing, DisableComments, FileUrl, FileType, TempTagList from ( select Auth0IDAuthor, Post.PostGuid, DateCreated, PostContent, Auth0IDWhoLiked, case when Auth0IDAuthor = Auth0IDWhoLiked then 'yes' else null end as DidUserLikePost, case when post.DisableSharing <> 'True' then 'False' end as DisableSharing, case when post.DisableComments <> 'True' then 'False' end as DisableComments from SM_Posts Post left join ( select * from SM_LikeCommentTable ) LikedComm on Post.Auth0IDAuthor = LikedComm.Auth0IDWhoLiked and Post.PostGuid = LikedComm.PostGuid ) a inner join SM_Account_Info AcctInfo on Auth0IDAuthor = AcctInfo.Auth0ID LEFT join SM_Follow_Following_Table on Auth0IDAuthor = SM_Follow_Following_Table.FollowingAuth0ID left join SM_Posts_File_Urls ON a.PostGuid = SM_Posts_File_Urls.PostGuid where SM_Follow_Following_Table.FollowerAuth0ID = @Auth0IDAuthor group by Auth0IDAuthor, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, a.PostGuid, DateCreated, PostContent, DidUserLikePost, DisableSharing, DisableComments, FileUrl, FileType, TempTagList ) subquery order by DateCreated desc";
                }

                else if (TypeOfPost == "ProfilePosts")
                {
                    SQL = " IF DBO.IsUserAFollower(@LoggedInUser, @Auth0IDAuthor) > 0 SELECT '-1' AS auth0idauthor ELSE /*profile posts*/ SELECT auth0idauthor, AcctInfo.ProfilePhotoUrl, a.postguid, AcctInfo.fullname, datecreated, postcontent, COUNT (auth0idwholiked) AS PostLikeCount, diduserlikepost, disablesharing, disablecomments, FileUrl, filetype, TempTagList /*, case when @ViewUserProfile <> '-1' then 'True' else 'False' end as ViewUserProfile */ FROM (SELECT auth0idauthor, Post.postguid, datecreated, postcontent, auth0idwholiked, CASE WHEN auth0idauthor = auth0idwholiked THEN 'yes' ELSE NULL END AS DidUserLikePost, CASE WHEN post.disablesharing <> 'True' THEN 'False' END AS DisableSharing, CASE WHEN post.disablecomments <> 'True' THEN 'False' END AS DisableComments FROM sm_posts Post LEFT JOIN (SELECT * FROM sm_likecommenttable) LikedComm ON Post.auth0idauthor = LikedComm.auth0idwholiked AND Post.postguid = LikedComm.postguid) a INNER JOIN sm_account_info AcctInfo ON auth0idauthor = AcctInfo.auth0id LEFT JOIN sm_posts_file_urls ON a.postguid = sm_posts_file_urls.postguid WHERE auth0idauthor = @Auth0IDAuthor GROUP BY auth0idauthor, AcctInfo.ProfilePhotoUrl, AcctInfo.fullname, a.postguid, datecreated, postcontent, diduserlikepost, disablesharing, disablecomments, fileurl, filetype, TempTagList ORDER BY DateCreated DESC  ";
                }
