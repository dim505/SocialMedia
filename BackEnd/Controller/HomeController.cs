...
";
        ...
        public IActionResult GetPosts(string TypeOfPost, string ViewUserProfile)
        {
            ...
            using (IDbConnection db = new SqlConnection(ConnStr))
            {
                ...
                {
                    SQL = "select * from ( select Auth0IDAuthor, a.PostGuid, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, DateCreated, PostContent, ( select count(*) from SM_LikeCommentTable where a.PostGuid = PostGuid ) as PostLikeCount, case when ( select count(*) from SM_LikeCommentTable where @Auth0IDAuthor = Auth0IDWhoLiked and a.PostGuid = PostGuid ) >= 1 then CAST(1 AS BIT) else CAST(0 AS BIT) end as DidUserLikePost, DisableSharing, DisableComments, FileUrl, FileType, TempTagList from ( select Auth0IDAuthor, Post.PostGuid, DateCreated, PostContent, case when post.DisableSharing <> 'True' then 'False' end as DisableSharing, case when post.DisableComments <> 'True' then 'False' end as DisableComments from SM_Posts Post ) a inner join SM_Account_Info AcctInfo on Auth0IDAuthor = AcctInfo.Auth0ID left join SM_Posts_File_Urls ON a.PostGuid = SM_Posts_File_Urls.PostGuid where Auth0IDAuthor = @Auth0IDAuthor group by Auth0IDAuthor, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, a.PostGuid, DateCreated, PostContent, DisableSharing, DisableComments, FileUrl, FileType, TempTagList union select Auth0IDAuthor, a.PostGuid, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, DateCreated, PostContent, ( select count(*) from SM_LikeCommentTable where a.PostGuid = PostGuid ) as PostLikeCount, case when ( select count(*) from SM_LikeCommentTable where @Auth0IDAuthor = Auth0IDWhoLiked and a.PostGuid = PostGuid ) >= 1 then CAST(1 AS BIT) else CAST(0 AS BIT) end as DidUserLikePost, DisableSharing, DisableComments, FileUrl, FileType, TempTagList from ( select Auth0IDAuthor, Post.PostGuid, DateCreated, PostContent, case when post.DisableSharing <> 'True' then 'False' end as DisableSharing, case when post.DisableComments <> 'True' then 'False' end as DisableComments from SM_Posts Post ) a inner join SM_Account_Info AcctInfo on Auth0IDAuthor = AcctInfo.Auth0ID LEFT join SM_Follow_Following_Table on Auth0IDAuthor = SM_Follow_Following_Table.FollowingAuth0ID left join SM_Posts_File_Urls ON a.PostGuid = SM_Posts_File_Urls.PostGuid where SM_Follow_Following_Table.FollowerAuth0ID = @Auth0IDAuthor group by Auth0IDAuthor, AcctInfo.FullName, AcctInfo.ProfilePhotoUrl, a.PostGuid, DateCreated, PostContent, DisableSharing, DisableComments, FileUrl, FileType, TempTagList ) subquery order by DateCreated desc";
                }

                else if (TypeOfPost == "ProfilePosts")
                {
                    SQL = "IF DBO.IsUserAFollower(@LoggedInUser, @Auth0IDAuthor) > 0 BEGIN SELECT '-1' AS auth0idauthor END ELSE /*profile posts*/ BEGIN SELECT auth0idauthor, AcctInfo.ProfilePhotoUrl, a.postguid, AcctInfo.fullname, datecreated, postcontent, ( select count(*) from SM_LikeCommentTable where a.PostGuid = PostGuid ) as PostLikeCount, case when ( select count(*) from SM_LikeCommentTable where @Auth0IDAuthor = Auth0IDWhoLiked and a.PostGuid = PostGuid ) >= 1 then CAST(1 AS BIT) else CAST(0 AS BIT) end as DidUserLikePost, disablesharing, disablecomments, FileUrl, filetype, TempTagList /*, case when @ViewUserProfile <> '-1' then 'True' else 'False' end as ViewUserProfile */ FROM ( SELECT auth0idauthor, Post.postguid, datecreated, postcontent, CASE WHEN post.disablesharing <> 'True' THEN 'False' END AS DisableSharing, CASE WHEN post.disablecomments <> 'True' THEN 'False' END AS DisableComments FROM sm_posts Post ) a INNER JOIN sm_account_info AcctInfo ON auth0idauthor = AcctInfo.auth0id LEFT JOIN sm_posts_file_urls ON a.postguid = sm_posts_file_urls.postguid WHERE auth0idauthor = @Auth0IDAuthor GROUP BY auth0idauthor, AcctInfo.ProfilePhotoUrl, AcctInfo.fullname, a.postguid, datecreated, postcontent, disablesharing, disablecomments, fileurl, filetype, TempTagList ORDER BY DateCreated DESC end ";
                }
